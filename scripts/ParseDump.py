...
# input: ./src/hcnet-core(+0xd9f6bd) [0x55ab4c2456bd]
def extract_relative_offset(line):
    if "hcnet-core" not in line:
        return ""

    # extracts (+0xd9f6bd) by matching on (+ ... )
    # ./src/hcnet-core(+0xd9f6bd) [0x55ab4c2456bd] -> (+0xd9f6bd)
...
def main():
    parser = argparse.ArgumentParser(
        description="Provide human readable stack trace for hcnet-core traces."
    )

    parser.add_argument(
        "exe",
        type=argparse.FileType("r"),
        help="Filepath to hcnet-core executable with debug symbols installed",
    )

    parser.add_argument(
        "stack_trace",
        help="Stack trace reported by hcnet-core (should start with something like: ./src/hcnet-core(+0xd9f6bd) [0x55a1fcb7d6bd])",
    )
