...
template <>
void
generator_t::operator()<hcnet::SCVal>(hcnet::SCVal& val) const
{
    ...
    const auto& vals = hcnet::SCVal::_xdr_case_values();
    hcnet::SCValType v;
...
template <>
void
generator_t::operator()<hcnet::SorobanAuthorizedInvocation>(
    hcnet::SorobanAuthorizedInvocation& auth) const
{
    if (++RECURSION_COUNT > RECURSION_LIMIT)
    {
        hcnet::SorobanAuthorizedInvocation a;
        auth = a;
        return;
    }

    xdr_traits<hcnet::SorobanAuthorizedInvocation>::load(*this, auth);
}
